...
class MyResNet(nn.Module):
    def __init__(self, num_classes=8, dim_in=73, dim_out=36,
        ...
        self.bn = nn.BatchNorm2d(dim_in, affine=True)
        ...
        self.multihead_attn = nn.MultiheadAttention(256, 8, batch_first=True, dropout=0.1)
        self.q_linear = nn.Linear(256,256)
        self.k_linear = nn.Linear(256,256)
        self.v_linear = nn.Linear(256,256)
    ...
    def forward(self, embed, atten):
        ...
        query = self.q_linear(embed)
        key = self.k_linear(embed)
        value = self.v_linear(embed)
        ls = []
        for j in range(4):
            attn_output, attn_output_weights = self.multihead_attn(query, key, value, average_attn_weights=False)
            query = self.q_linear(attn_output)
            key = self.k_linear(attn_output)
            value = self.v_linear(attn_output)
            ls.append(attn_output_weights)

        embed = torch.concat(ls, dim=1)   # (batch_size, 64, L, L)
        # 首先对embed做transformer_encoder, 然后通过注意力机制将1d信息转化为2d
        ...
        Y = self.linear(Y)
        # Y: (batch, len, len, dim_out) -> (batch, len, len, num_classes)   num_classes: 8
...
if __name__ == '__main__':
    ...
    # print(output.shape)  # [batch*L*L,dim_out], [2*129*129,8]
